...
The response uses `Cache-Control: no-store` and returns the raw upstream body, parsed contract when valid, request ID, token usage, exact repository pricing-basis cost, deterministic raster checkpoints and exact 5 pt preflight. The caller is responsible for saving this existing-owner response locally during a separately authorized live phase, then removing the temporary route.
